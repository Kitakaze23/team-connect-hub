...
serve(async (req) => {
  ...
  try {
    ...
    if (companyError || !company || company.owner_id !== user.id) {
      return new Response(JSON.stringify({ error: "Not the company owner" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // Approve the owner's membership and set role to admin
